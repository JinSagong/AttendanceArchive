...
@Composable
fun FilingFruitScreen(filingStateModel: FilingStateModel) {
    val coroutineScope = rememberCoroutineScope()
    val scrollState = rememberScrollState()
    if (ScreenManager.filingFruitScreen.value == Pair(true, true)) {
        coroutineScope.launch { scrollState.scrollTo(0) }
        filingStateModel.initPosition()
    }
    SideEffect {
        if (ScreenManager.filingFruitScreen.value == Pair(false, false)) {
            filingStateModel.cancelGenerateFile()
        }
    }

    AnimatedVisibility(
        visible = ScreenManager.filingFruitScreen.value.first,
        modifier = Modifier.fillMaxSize()
            .addFocusCleaner(LocalFocusManager.current),
        enter = slideInHorizontally(initialOffsetX = { if (ScreenManager.filingFruitScreen.value.second) it else -it }),
        exit = slideOutHorizontally(targetOffsetX = { if (ScreenManager.filingFruitScreen.value.second) -it else it })
    ) {
                ...
                Column(
                    ...
                    Column(modifier = Modifier.fillMaxWidth().wrapContentHeight()) {
                        CheckBoxRow(
                            ...
                            text = Strings.filingFruitWeekly,
                            onClick = { filingStateModel.option.value = 0 },
                        )
                        CheckBoxRow(
                            modifier = Modifier.wrapContentSize(),
                            checked = filingStateModel.option.value == 1,
                            text = Strings.filingFruitMonthly,
                            onClick = { filingStateModel.option.value = 1 },
                        )
                        CheckBoxRow(
                            modifier = Modifier.wrapContentSize(),
                            checked = filingStateModel.option.value == 2,
                            text = Strings.filingFruitYearly,
                            onClick = { filingStateModel.option.value = 2 },
                        )
                    }
                    Button(modifier = Modifier.fillMaxWidth().wrapContentHeight()
                        ...
                        onClick = {
                            ...
                            filingStateModel.generateFruitFile(coroutineScope)
                        }) {
                        Text(
                            modifier = Modifier.fillMaxWidth().wrapContentHeight()
                                .padding(0f.dp, 4f.dp),
